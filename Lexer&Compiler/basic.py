...
class ParseResult:
    ...
    def  try_register(self, res):
        if(res.error):
            self.to_reverese_count = res.advance_count
            return None
...
###########################################
    ...
    def statements(self):
        ...
        while(True):
            ...
            if(not statement):
                self.reverse(res.to_reverese_count)
                more_statements = False
                continue
            statements.append(statement)
    ...
    def for_expr(self):
        ...
        if(res.error): return ResourceWarning
...
class BaseFunction(Value):
    ...
    def check_args(self, arg_names, args):
        ...
        if(len(args) < len(arg_names)):
            return res.failure(RTError(
                self.pos_start, self.pos_end,
                f"{len(arg_names) - len(args)} too many args passed into {self}",
                self.context
            ))
...
class Function(BaseFunction):
    ...
    def execute(self, args):
        ...
        if(res.error): return res
        return res.succes(Number.null if self.should_return_null else value)
...
class BuiltInFunction(BaseFunction):
    ...
    def no_visit_method(self, node, context):
        raise Exception(f'No exectue_{self.name} method defined')
